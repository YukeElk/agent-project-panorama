...
在本目录中运行 `pnpm install --frozen-lockfile`、`pnpm build`。`pnpm test:all` 检查过程工具、工作台和离线分发；测试所用 Python 绝对路径通过 `PANORAMA_TEST_PYTHON` 指定。P0 的独立合同测试另需 Python jsonschema，仅用于开发验证。
